...
class Table:
    ...
    def add_columns(self):
        df = pd.DataFrame()
        df["brand"] = self.df["name"].str.split(" ", n=1).str.get(0)
        df["product name"] = self.df["name"].str.split(" ", n=1).str.get(1)
        df["link"] = "<a href='result.py?BRAND=" + self.df["brand"] + "&MB=" + self.df["product name"] + "'>Search</a>"
        self.df = df 
    #dfをhtml化
    def to_html(self):
