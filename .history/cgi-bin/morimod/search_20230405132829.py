...
def search_google(BRAND,MB):
    ...
    #見出しにマザーボード名を含む
    h=driver.find_elements(By.XPATH,"//h[contains(@.,'{}')]".format(MB.lower()))
    
    #URLの一致はこれ
    #h = driver.find_elements(By.XPATH,"//a[contains(@href,'{}')]".format(BRAND.lower()))
    
    for elem in h:
            print("elemのhref:{}".format(elem.get_attribute('href')))
            #print("elemの結果:{}".format(elem.find_element(By.XPATH,"./h3").text))
    """
    for elem in h:
        print(elem)
