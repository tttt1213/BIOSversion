...
def BMC(BRAND,MB):
    ...
    if file_element is not None:
        ...
        filename = "No file Found"
        fileURL = "https://www.google.com/search?q={}+{}+BMC+IPMI".format(BRAND,MB)
...
def search_google(BRAND,MB):
    ...
    # 検索結果の最初のリンクを取得する
    first_link = driver.find_elements(By.XPATH, "//a[contains(text(),{})]".format(MB.lower()) or "//a[contains(text(),{})]".format(MB) )
    first_link.get_attribute('href')
    first_link = driver.find_elements(By.XPATH, "//a[contains(@href,{})]".format(BRAND.lower()))
